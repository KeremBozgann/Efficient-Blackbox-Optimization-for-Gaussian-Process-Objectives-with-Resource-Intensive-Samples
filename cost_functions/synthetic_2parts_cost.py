...
def synthetic_2parts_cost(X):
    ...
    for i in range(X.shape[0]):
        ...
        if xi1 > 0:
            ...
            result[i, 0] = 1+ np.exp(-(xi1 ** 2 + xi2 ** 2))
        else:
            result[i, 0] = 1+ 4*np.exp(-(xi1 ** 2 + xi2 ** 2))

    return result

def scipy_minimize_synthetic_2parts_cost():
...
def synthetic_2parts_cost_plots(disc, plot= False):
...
def synthetic_2parts_cost_find_best_suited_kernel(X, Y, noise=10**(-4)):
