...
class Ei_ppu_optimize:
    ...
    def ei_ppu_negative(self, x, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt):

        x = x.reshape(1, -1)

        ei_pu_avg= 0
        self.cost_list= np.empty([self.n, 1])
        ...
        for k in range(self.n):
            ...
            self.sigma_x_lat_cost= np.sqrt(self.var_x_lat_cost)
            costk= np.exp(self.u_x_lat_cost+z*self.sigma_x_lat_cost)
            self.cost_list[k, :]= costk[0, :]
            ei_puk= ei(self.sigma_x, self.u_x, f_best)/costk
            ei_puk *= 1 / np.sqrt(np.pi) * w
            ei_pu_avg+= ei_puk

        ei_pu_avg= ei_pu_avg.flatten()

        return -ei_pu_avg


    def grad_ei_ppu_negative(self, x, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt):
        ...
        for k in range(self.n):
            ...
            costk = self.cost_list[k, :].reshape(1, -1)

            grad_cost= costk*(self.du_x_lat_cost+ z*self.dsigma_x_lat_cost)

            grad_puk = (costk*grad_ei_x- grad_cost*ei_x)/costk**2
            grad_puk *= 1 / np.sqrt(np.pi) * w
            grad_pu_avg += grad_puk

        grad_pu_avg = grad_pu_avg.flatten()

        return -grad_pu_avg

    def maximize_ei_ppu(self, kernel, latent_cost_kernel, Xt, Yt, noise, model, latent_cost_model, domain, f_best,
                             num_outer_opt_restarts, grid_opt_out, D, Yt_latent_cost, num_iter_max, noise_cost):
        ...
        if num_outer_opt_restarts > 0:
            ...
            for i in range(num_outer_opt_restarts):
                ...
                result = (minimize(self.ei_ppu_negative, x0, args= fun_args, bounds=b, method='L-BFGS-B', jac= self.grad_ei_ppu_negative,
        ...
        if grid_opt_out== True:
            ...
            if D==1:
                ...
                for j in range(disc):
                    ...
                    x_cand_value= -self.ei_ppu_negative(x_cand, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt);
...
def test_ei_ppu():
    ...
    x1_test_val= opt.ei_ppu_negative(x1_test, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)

    x1_test_val*=-1
    '''analytical gradient'''
    x1_test_grad= -opt.grad_ei_ppu_negative(x1_test, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)
    ...
    x11_val = -opt.ei_ppu_negative(x11, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)

    x12_val = -opt.ei_ppu_negative(x12, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)
    x21_val = -opt.ei_ppu_negative(x21, model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)
    x22_val = -opt.ei_ppu_negative(x22,  model, latent_cost_model, f_best, Yt_latent_cost, kernel, latent_cost_kernel,
                         noise_cost, Xt, noise, Yt)
