...
def EI_bo_cont_domain_nocost(input_dimension, output_dimension, objective_func, y_true_opt,
                      x_true_opt, domain, kernel, evaluation_budget, x0, grid, noise=10**(-4),
                             opt_restarts= 10, random_restarts=10, num_iter_max= 100, plot=False):
    ...
    while t<evaluation_budget:
        ...
        if t==0:
            ...
            xt, xt_value = EI_optimize_with_gradient(domain, model, opt_restarts, num_iter_max,
                                                     Xt, Yt, f_best, kernel, noise, D, grid)
...
def test_ei(input_dimension, output_dimension, objective_func, y_true_opt,
            x_true_opt, domain, kernel, evaluation_budget, x0, noise, plot, num_iter_max,
            opt_restarts, random_restarts, grid):

    loss_list, Xt, Yt, model= \
        EI_bo_cont_domain_nocost(input_dimension, output_dimension, objective_func, y_true_opt,
                                 x_true_opt, domain, kernel, evaluation_budget, x0, grid, noise=noise,
                                 opt_restarts=opt_restarts, random_restarts=random_restarts,
                                 num_iter_max=num_iter_max, plot=plot)
...
# grid= True
#
# loss_list, Xt, Yt, model= test_ei(input_dimension, output_dimension, objective_func, y_true_opt,
#             x_true_opt, domain, kernel, evaluation_budget, x0, noise, plot, num_iter_max,
#             opt_restarts, random_restarts, grid)
