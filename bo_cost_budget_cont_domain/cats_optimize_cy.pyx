...
def funfi(np.ndarray A):
    cdef np.ndarray result
    result= np.tril(A, k=-1)+ 1/2*np.diagflat(np.diag(A))
    return result

def get_posterior_covariance_and_derivative_1q(np.ndarray x, np.ndarray x1, np.ndarray Xt, kernel, float noise):

    #K_x1_x= K_x_x1
    cdef np.ndarray K_inv, k_x1_Xt, k_x_Xt_T, alpha, k_x1_x, K_x1_x, diff_vec, term1, term2, dK_x1_x_dx1
...
def get_dsigma0_dx1_q1(np.ndarray x2_opt, np.ndarray x1, np.ndarray Xt, kernel, float noise,
                            np.ndarray dK0_x1_dx1, int D, np.ndarray L0_x1, np.ndarray L0_x1_inv, np.ndarray L0_x1_inv_T):

    #K0_x1: 1*1, dK0_x1_dx1: D*1

    cdef np.ndarray K0_x2_x1, dK0_x2_x1_dx1, dL0_x1_dx1=np.empty([D, 1]), dK0_x1_dx1i, Ai, temp, dL0_x1_dx1i, \
                        dL0_x1_inv_dx1= np.empty([D,1]), dL0_x1_inv_dx1i, dsigma0_dx1= np.empty([D,1]), \
                        dK0_x2_x1_dx1i, dsigma0_dx1i, sigma0
...
def get_lookahead_mean_and_variance_gradient_wrt_x1(np.ndarray dsigma0_dx1, np.ndarray sigma0, np.ndarray Z, int D):

    cdef np.ndarray du1_x2_dx1, dvar1_x2_dx1= np.empty([D,1]), dsigma0_dx1i, dvar1_x2_dx1i
    cdef int i
...
def get_df_best1_dx1(double f_best, np.ndarray Z, np.ndarray du0_x1_dx1, np.ndarray dL0_x1_dx1, double y1):

    cdef np.ndarray best1_grad

    if (f_best- y1) <0.0:

        best1_grad= np.atleast_2d(0.0)
...
def get_one_step_mean_and_variance(np.ndarray u0_x2, np.ndarray var0_x2, np.ndarray Z, np.ndarray sigma0):

    cdef np.ndarray u1_x2, var1_x2
...
def dnormal_dfbest(double f_best, np.ndarray u_x, np.ndarray sigma_x):


    return -(f_best - u_x) / (sigma_x** 2) * norm.pdf((f_best - u_x) / sigma_x)

def dcumulative_dfbest(double f_best, np.ndarray sigma_x, np.ndarray u_x):

    return 1/sigma_x*norm.pdf((f_best- u_x)/sigma_x)

def cats_dEI1_x2_dx1(double f_best, double f_best1,  kernel, np.ndarray Xt, np.ndarray Yt, double noise, np.ndarray x2_opt,
                        np.ndarray x1, int D, np.ndarray Z, np.ndarray u0_x2, np.ndarray var0_x2, np.ndarray L0_x1,
                        np.ndarray L0_x1_inv, np.ndarray L0_x1_inv_T, double y1):

    cdef np.ndarray du0_x1_dx1, dK0_x1_dx1, K0_x1, dsigma0_dx1, sigma0, dL0_x1_dx1, du1_x2_opt_dx1, dK1_x2_opt_dx1,\
                     best1_grad_x1_, u1_x2_opt, var1_x2_opt,  sigma1_x2_opt, dsigma1_x2_opt_dx1, dcumulative_dx, \
                     term1, term2, term3, term4, dnormal_dx
    cdef float f_best_fl
    ...
    f_best_fl= float(f_best)
    best1_grad_x1= get_df_best1_dx1(f_best_fl, Z, du0_x1_dx1, dL0_x1_dx1, y1)
...
class cats_optimize():
    ...
    def update_x1_dependent(self, model, latent_cost_model, np.ndarray x1):

        cdef np.ndarray u0_x1, var0_x1, sigma0_x1, u0_x1_cost

        u0_x1, var0_x1, sigma0_x1= get_mean_var_std(x1, model)
        self.u0_x1, self.var0_x1, self.sigma0_x1= u0_x1, var0_x1, sigma0_x1

        u0_x1_cost = get_mean_of_cost(x1, latent_cost_model)
        self.u0_x1_cost= u0_x1_cost

    def update_z_dependent(self, np.ndarray z, double f_best, np.ndarray Xt, np.ndarray Yt, np.ndarray x1):
    ...
    def cats_negative_improvement(self, np.ndarray x1, kernel, latent_cost_kernel, np.ndarray z, int num_inner_opt_restarts,
                                   int grid_opt_in, int D, np.ndarray Xt, np.ndarray Yt, double noise, model, latent_cost_model,
                                 list domain, Yt_latent_cost, int num_iter_max, double f_best, double noise_cost):

        cdef np.ndarray CATS_x1, x2_opt, two_opt
    ...
    def cats_negative_gradient(self, np.ndarray x1, kernel, latent_cost_kernel, np.ndarray z, int num_inner_opt_restarts,
                                int grid_opt_in, int D, np.ndarray Xt, np.ndarray Yt, double noise, model, latent_cost_model,
                                 list domain, np.ndarray Yt_latent_cost, int num_iter_max, double f_best, double noise_cost):

        cdef np.ndarray CATS_grad
        x1 = x1.reshape(1,-1)
    ...
    def set_monte_carlo_parameters(self, int monte_carlo_samples):

        cdef np.ndarray Z_list
        self.monte_carlo_samples= monte_carlo_samples
        Z_list = np.random.normal(0.0, 1.0, (1, monte_carlo_samples))
        self.Z_list= Z_list

    def monte_carlo_average_two_step_pu_improvement(self, np.ndarray x1, model, latent_cost_model, double f_best, np.ndarray Xt,
                                                np.ndarray Yt, kernel, latent_cost_kernel, int num_inner_opt_restarts, int grid_opt_in,
                                                 double noise, list domain, np.ndarray Yt_latent_cost, int num_iter_max, int D):

        cdef np.ndarray x2_opt_list_mont, x2_opt_two_opt_list_mont, z, negative_improvement_puk, x2_opt, x2_opt_two_opt
        cdef double avg_negative_improvement_pu= 0.0
        cdef int k
        x1 = x1.reshape(1, -1)
        #avg_negative_improvement_pu= 0.0


        x2_opt_list_mont= np.zeros([self.Z_list.shape[1], D])
        x2_opt_two_opt_list_mont= np.zeros([self.Z_list.shape[1], 1])
        self.x2_opt_list_mont= x2_opt_list_mont
        self.x2_opt_two_opt_list_mont= x2_opt_two_opt_list_mont
    ...
    def monte_carlo_average_two_step_pu_gradient(self, x1, model, latent_cost_model, f_best, Xt, Yt, kernel, latent_cost_kernel,
        ...
        cdef np.ndarray z, x2_opt, x2_opt_two_opt, negative_improvement_puk
        cdef double avg_negative_gradient_pu = 0
        cdef int k
        ...
        for k in range(self.Z_list.shape[1]):
            x2_opt= self.x2_opt_list_mont[k, :].reshape(1,-1)
            self.x2_opt = x2_opt
            x2_opt_two_opt= self.x2_opt_two_opt_list_mont[k, :].reshape(1,-1)
            self.x2_opt_two_opt= x2_opt_two_opt
            z = self.Z_list[:, k]
            z= np.atleast_2d(z)
    ...
    def set_hermite_gauss_parameters(self, int n):
    ...
    def hermite_gaussian_app_two_step_pu_improvement(self, x1, D, model, latent_cost_model, f_best, Xt, Yt, kernel, latent_cost_kernel,
        ...
        cdef np.ndarray x2_opt_list, x2_opt_two_opt_list_herm, z, negative_improvement_puk, x2_opt, x2_opt_two_opt
        cdef int k
        #cdef np.float z_fl, w
        ...
        x2_opt_list= np.zeros([len(self.Z), D])
        x2_opt_two_opt_list_herm = np.zeros([len(self.Z), 1])
        self.x2_opt_list=  x2_opt_list
        self.x2_opt_two_opt_list_herm= x2_opt_two_opt_list_herm

        self.update_x1_dependent(model, latent_cost_model, x1)

        for k in range(len(self.Z)):
            # print('k:{}'.format(k))
            z_fl= self.Z[k]; w= self.W[k]
            z_fl*= np.sqrt(2)
            z= np.atleast_2d(z_fl)
            self.update_z_dependent(z, f_best, Xt, Yt, x1)
    ...
    def hermite_gaussian_app_two_step_pu_gradient(self, x1, D, model, latent_cost_model, f_best, Xt, Yt, kernel, latent_cost_kernel,
        ...
        cdef np.ndarray x2_opt, x2_opt_two_opt, z, negative_gradient_puk
        cdef int k
        #cdef np.float z_fl, w
        ...
        for k in range(len(self.Z)):
            ...
            x2_opt= self.x2_opt_list[k, :].reshape(1,-1)
            self.x2_opt= x2_opt
            x2_opt_two_opt = self.x2_opt_two_opt_list_herm[k, :].reshape(1, -1)
            self.x2_opt_two_opt= x2_opt_two_opt
            ...
            z = np.atleast_2d(z_fl)
            self.update_z_dependent(z, f_best, Xt, Yt, x1)
            negative_gradient_puk = self.cats_negative_gradient(x1, kernel, latent_cost_kernel, z,
                                num_inner_opt_restarts, grid_opt_in, D, Xt, Yt, noise, model, latent_cost_model,
                                                               domain, Yt_latent_cost, num_iter_max, f_best, noise_cost)

            negative_gradient_puk *= 1/np.sqrt(np.pi)*w
            negative_gradient_pu += negative_gradient_puk

        return negative_gradient_pu

    def optimize_cats(self, int num_outer_opt_restarts, list domain, int num_iter_max, bint grid_opt_out, int D, model,
                    latent_cost_model, double f_best, np.ndarray Xt, np.ndarray Yt, kernel, latent_cost_kernel,
                        int num_inner_opt_restarts, bint grid_opt_in, double noise, np.ndarray Yt_latent_cost, int n,
                           double noise_cost):

        cdef int i, index_opt_x1_sci, disc, j
        cdef np.ndarray lower= np.empty(D,), upper= np.empty(D,), x10, x1_cand_sci, cand_value_x1_sci, x1_cand_grad_sci, \
                        x1_opt_sci, x10_opt_sci, x1_opt_grad_sci, x1_opt_value_sci, x1_grid, X_grid, x1_value_list_grid, x1_cand, \
                         x1_cand_value, index_max_grid, x_opt_value_grid, x_opt_grid, x2_grid, X1_grid, X2_grid, X1_flat, X2_flat
        #cdef np.float x1_max, x2_max, x1_min, x2_min
        ...
        if num_outer_opt_restarts > 0:
            ...
            #lower= np.empty(D,); upper= np.empty(D,)
            ...
            x1_opt_list_sci = np.empty([num_outer_opt_restarts, D])
            opt_value_list_x1_sci = np.empty([num_outer_opt_restarts, 1])
            grad_list_x1_sci = np.empty([num_outer_opt_restarts, D])
            x10_list_sci = np.empty([num_outer_opt_restarts, D])
            ...
            for i in range(num_outer_opt_restarts):
                ...
                x1_opt_list_sci[i, :]= x1_cand_sci[0,:]
                opt_value_list_x1_sci[i, :]= cand_value_x1_sci[0, :]
                grad_list_x1_sci[i, :] = x1_cand_grad_sci[0,:]
                x10_list_sci[i,:]= x10[0,:]

            index_opt_x1_sci = int(np.argmax(np.array(opt_value_list_x1_sci)))
            x1_opt_sci = x1_opt_list_sci[index_opt_x1_sci, :].reshape(1,-1)
            x1_opt_sci= x1_opt_sci.reshape(1,-1)
            x1_opt_value_sci = opt_value_list_x1_sci[index_opt_x1_sci, :].reshape(1,-1)
            x1_opt_grad_sci = grad_list_x1_sci[index_opt_x1_sci, :].reshape(1,-1)
            x10_opt_sci = x10_list_sci[index_opt_x1_sci, :].reshape(1,-1)
...
#
#     num_inner_opt_restarts= 10; num_outer_opt_restarts=1; D=1;
#     grid_opt_in= False; grid_opt_out= False
#     num_iter_max = 0;
#     n=20
