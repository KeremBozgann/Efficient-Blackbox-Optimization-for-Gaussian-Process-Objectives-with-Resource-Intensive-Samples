...
def temp_test_speed_1d():
    ...
    num_inner_opt_restarts= 10; num_outer_opt_restarts=10;
    grid_opt_in= False; grid_opt_out= False
    num_iter_max = 0;
    n=20

    t1_cy= time.clock()
    opt= cats_optimize_cy.cats_optimize()
    ...
    # print('unoptimized py code time:{}, optimized py code time:{}, cy code time{}'.format((t2unoptimized_py- t1_unoptimized_py),
    #                                                                       (t2_optimized_py- t1_optimized_py), (t2_cy- t1_cy)))

    print('optimized py code time:{}, cy code time{}'.format((t2_optimized_py- t1_optimized_py), (t2_cy- t1_cy)))

def temp_test_speed_2d():

    _, __, domain = ackley_opt()
    D = 5; noise = 10 ** (-3)
...
def temp_test_speed_5d():

    _, __, domain = ackley_opt(5)
    D = 5; noise = 10 ** (-3)
    _, __, domain_cost= exp_cos_5d_opt()
    noise_cost = 10 ** (-3)
    ...
    Xt= np.random.uniform(lower, upper, (3,D))
    Yt= ackley(Xt)
    Yt_cost= exp_cos_5d(Xt); log_Yt_cost= np.log(Yt_cost); Yt_latent_cost= log_Yt_cost.copy()

    X_grid_0_ack, Y_ack=  ackley_grid()
    X_grid_0_cos, Y_cos= exp_cos_5d_grid()
    latent_cost_kernel= exp_cos_5d_find_best_suited_kernel(X_grid_0_cos, Y_cos, noise=10 ** (-4))
    kernel = ackley_find_best_suited_kernel(X_grid_0_ack, Y_ack, noise=10**(-4))
    ...
    #optimized py code time:26083.0624849, cy code time26775.123940899997
