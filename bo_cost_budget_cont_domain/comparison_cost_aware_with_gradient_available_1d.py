...
X_kern, Y_kern = sin_plots(disc, plot=False)
model, kernel = sin_find_best_suited_kernel(X_kern, Y_kern, noise=noise)

X_cost_kern, Y_cost_kern = exp_cos_1d_plots(disc, False)
latent_cost_model, latent_cost_kernel = exp_cos_1d_find_best_suited_kernel(X_cost_kern, Y_cost_kern, noise=noise_cost)
...
count_carbo= np.zeros(cost_grid.shape); loss_carbo= np.zeros(cost_grid.shape);
...
count_ppu= np.zeros(cost_grid.shape); loss_ppu= np.zeros(cost_grid.shape);
count_prox= np.zeros(cost_grid.shape); loss_prox= np.zeros(cost_grid.shape);
...
for i in range(total_iterations):
    ...
    '''ei proximity search'''
    loss_list_prox, Xt_prox, Yt_prox, model_prox, cost_list_prox, cum_cost_list_prox, latent_cost_kernel_prox= \
    ...
    loss_prox, count_prox = add_loss(loss_prox, count_prox
                                       ,loss_list_prox, np.atleast_1d(np.array(cum_cost_list_prox).squeeze()),
                                       cost_grid)
    print('entering ppu')
    '''ei_ppu'''
    loss_list_ppu, Xt_ppu, Yt_ppu, model_ppu, cost_list_ppu, cum_cost_list_ppu, latent_cost_kernel_ppu= \
    ...
    loss_ppu, count_ppu = add_loss(loss_ppu, count_ppu, loss_list_ppu, np.atleast_1d(np.array(cum_cost_list_ppu).squeeze()),
                                    cost_grid)
    ...
    '''carbo'''
    loss_list_carbo, Xt_carbo, Yt_carbo, model_carbo, cost_list_carbo, cum_cost_list_carbo, latent_cost_kernel_carbo = \
        carbo_bo_cont_domain(D, objective_func, cost_function, y_true_opt, x_true_opt,
                             ...
                             noise= noise, noise_cost= noise_cost, plot=False, plot_cost=False, num_layer= num_layer,
                             num_dense=num_dense, hyper_opt_per=hyper_opt_per, X_init= None, Y_init= None, Y_cost_init=Y_cost)

    loss_list_carbo = np.log10(loss_list_carbo)
    loss_carbo, count_carbo = add_loss(loss_carbo, count_carbo
                                   ,loss_list_carbo, np.atleast_1d(np.array(cum_cost_list_carbo).squeeze()), cost_grid)

    print('entering ei_pu')
    '''ei_pu'''
    loss_list_ei_pu, Xt_ei_pu, Yt_ei_pu,model_ei_pu, cost_list_ei_pu, cum_cost_list_ei_pu, latent_cost_kernel_pu = \
    ...
    loss_ei_pu, count_ei_pu = add_loss(loss_ei_pu, count_ei_pu
                                   ,loss_list_ei_pu, np.atleast_1d(np.array(cum_cost_list_ei_pu).squeeze()), cost_grid)

    print('entering ei')
    '''ei'''
    loss_list_ei, Xt_ei, Yt_ei, model_ei, coss_list_ei, cum_cost_list_ei, latent_cost_kernel_ei = \
    ...
    loss_ei, count_ei = add_loss(loss_ei, count_ei
    ...
    loss_and_count_dict = {'ei_pu': [loss_ei_pu, count_ei_pu, cost_grid], 'carbo': [loss_carbo, count_carbo, cost_grid],
                           'ei_ppu': [loss_ppu, count_ppu, cost_grid], 'ei': [loss_ei, count_ei, cost_grid],
                           'ei_prox': [loss_prox, count_prox, cost_grid]}

    save_to_hf(loss_and_count_dict, hf_name, i)
...
parameter_dict= {'ei_pu':
         {'random_restarts':random_restarts_ei_pu, 'num_iter_max':num_iter_max_ei_pu, 'grid':grid_ppu},
                'carbo':
        {'random_restarts':random_restarts_carbo, 'num_iter_max':num_iter_max_carbo, 'grid':grid_carbo},
                 'ei_ppu':
         {'random_restarts':random_restarts_ei_ppu, 'num_iter_max':num_iter_max_ei_ppu, 'grid':grid_ppu},
                 'ei':
         {'random_restarts': random_restarts_ei, 'num_iter_max': num_iter_max_ei, 'grid': grid_ei},
                 'ei_prox':
         {'random_restarts': random_restarts_prox, 'num_iter_max': num_iter_max_prox, 'grid': grid_prox}
                     }
                 # 'pucla_v2':
