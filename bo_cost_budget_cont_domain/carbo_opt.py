...
def  carbo_optimize_with_gradient(C, budget, c, domain, model, latent_cost_model, random_restarts, num_iter_max, Xt, Yt, Yt_cost, f_best,
                                 kernel, latent_cost_kernel, noise, D, grid):


    def carbo_negative(x):
        ...
        EI_cool_x= EI_cool(sigma_x, u_x, f_best, u_x_cost, C, budget, c)
        EI_cool_x= EI_cool_x.flatten()

        return -EI_cool_x


    def carbo_negative_gradient(x):
        ...
        expon= (budget - C) / (budget - c)
        denom= (u_x_cost) ** expon #c= init_cos
        ...
        grad_denom= expon* (u_x_cost) ** (expon -1)* du_x_cost

        '''overall gradient'''
        ei_x= EI(sigma_x, u_x, f_best)
        grad_x= (denom*grad_nom- grad_denom* ei_x)/(denom**2)
    ...
    if random_restarts>0:
        ...
        for i in range(random_restarts):
            ...
            result= minimize( carbo_negative, x0, bounds=b,  method='L-BFGS-B',
                              jac=carbo_negative_gradient, options= {'maxiter': num_iter_max})
    ...
    if D==1 and grid:
        ...
        EI_cool_X_grid = EI_cool(sigmaX_grid, uX_grid, f_best, u_X_cost_grid, C, budget, c)

        index_max_grid= int(np.argmax(EI_cool_X_grid, axis=0))
        x_opt_value_grid= EI_cool_X_grid[index_max_grid, :].reshape(1,-1)
...
def temp_test_carbo_gradient():
    budget= 20
    C= np.random.uniform(0, budget/2)
    c=budget/8
    ...
    x_test_val = -carbo_negative(x_test)
    grad_x_test= - carbo_negative_gradient(x_test)
    x_pl = x_test + 0.00001; x_mn = x_test - 0.00001;

    x_pl_val= -carbo_negative(x_pl)
    x_mn_val= -carbo_negative(x_mn)
