...
def ei_ppu_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                 domain, kernel, budget, x0, latent_cost_kernel,num_outer_opt_restarts,  grid_opt_out, noise=10**(-4),
                    noise_cost= 10**(-4), num_iter_max= 100, plot=False, plot_cost= False, num_layer= None,
    ...
    while C<budget:
        ...
        if Xt.shape[0]== 0:
            ...
            if objective_func == 'fashion':
                ...
                yt, yt_cost, xt = fashion_model.evaluate_error_and_cost(layer_sizes, alpha, l2_regul, 1.7)
            ...
            xt, xt_val, x1_value_list_grid, X1_grid = \
                opt.maximize_ei_ppu(kernel, latent_cost_kernel, Xt, Yt, noise, model, latent_cost_model, domain, f_best,
                             num_outer_opt_restarts, grid_opt_out, D, log_Yt_cost, num_iter_max, noise_cost)
            ...
            if objective_func == 'fashion':
                ...
                dropout=xt[0,z+2]
                yt, yt_cost, xt= cifar_model.evaluate_error_and_cost(filter_sizes, dense_sizes, alpha, l2_regul, dropout, num_epoch=0)
...
def test_synthetic():
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list = ei_ppu_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                 domain, kernel, budget, x0, latent_cost_kernel,num_outer_opt_restarts,  grid_opt_out, noise= noise,
                    noise_cost=  noise_cost, num_iter_max= 100, plot= True, plot_cost= True, num_layer= None, num_dense= None,
                                                                   hyper_opt_per= False)
def test_synthetic2():
...
def test_cifar():
    ...
    budget= 10*60; num_outer_opt_restarts= 10;
    grid_opt_out= False
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list = ei_ppu_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                 domain, kernel, budget, x0, latent_cost_kernel,num_outer_opt_restarts,  grid_opt_out, noise= noise,
                    noise_cost=  noise_cost, num_iter_max= 100, plot= False, plot_cost= False, num_layer= num_layer, num_dense= num_dense,
                                                                   X_init= X, Y_init= Y, Y_cost_init= Y_cost)
...
def test_fashion():
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list = ei_ppu_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                 domain, kernel, budget, x0, latent_cost_kernel, num_outer_opt_restarts,  grid_opt_out, noise= noise,
                    noise_cost=  noise_cost, num_iter_max= 100, plot=False, plot_cost= False, num_layer= num_layer, num_dense= None,
                                                                   X_init= X, Y_init= Y, Y_cost_init= Y_cost, hyper_opt_per= 5)