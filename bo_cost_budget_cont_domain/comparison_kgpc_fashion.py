...
'''kgpc parameters'''
num_inner_opt_restarts_kgpc= 10
num_outer_opt_restarts_kgpc= 10
num_iter_max_kgpc= 100
grid_opt_out_kgpc= False
grid_opt_in_kgpc= False
...
count_kgpc= np.zeros(cost_grid.shape); loss_kgpc= np.zeros(cost_grid.shape); f_best_kgpc= np.zeros(cost_grid.shape)
...
for i in range(total_iterations):
    ...
    if objective_func=='fashion':
        x0= None
        X, Y, Y_cost = initial_training_fashion(domain,  num_init_train_samples, num_layer, num_epoch)

    elif objective_func=='cifar':
        x0= None
        X, Y, Y_cost = initial_training_cifar(domain, num_init_train_samples, num_layer, num_dense, num_epoch)
    ...
    loss_list_carbo, Xt_carbo, Yt_carbo, model_carbo, cost_list_carbo, cum_cost_list_carbo, latent_cost_kernel_carbo, f_best_list_carbo = \
        carbo_bo_cont_domain(D, objective_func, cost_function, y_true_opt, x_true_opt,
                             ...
                             num_dense=num_dense,num_epoch= num_epoch, hyper_opt_per=hyper_opt_per, X_init=X, Y_init=Y, Y_cost_init=Y_cost)
    ...
    print('entering kgpc')
    '''kgpc'''
    loss_list_kgpc, Xt_kgpc, Yt_kgpc, model_kgpc, cost_list_kgpc, cum_cost_list_kgpc, latent_cost_kernel_kgpc, f_best_list_kgpc = \
        kgpc_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                domain, kernel, budget, x0, latent_cost_kernel, num_outer_opt_restarts_kgpc, grid_opt_out_kgpc, grid_opt_in_kgpc,
                num_inner_opt_restarts_kgpc,
                noise=noise, noise_cost=noise_cost, num_iter_max=num_iter_max_kgpc, plot=False, plot_cost=False,
                num_layer=num_layer,
                num_dense=num_dense, num_epoch= num_epoch, X_init=X, Y_init=Y, Y_cost_init=Y_cost, hyper_opt_per= hyper_opt_per)

    loss_list_kgpc = np.log10(loss_list_kgpc)
    # f_best_list_carbo = np.log10(f_best_list_carbo)
    loss_kgpc, count_kgpc, f_best_kgpc = add_loss(loss_kgpc, count_kgpc
                                   ,loss_list_kgpc, np.atleast_1d(np.array(cum_cost_list_kgpc).squeeze()),
                                                     f_best_list_kgpc, f_best_kgpc, cost_grid)
    ...
    loss_and_count_dict= {'ei_pu':[loss_ei_pu, count_ei_pu, cost_grid, f_best_pu],
                          ...
                          'kgpc': [loss_kgpc, count_kgpc, cost_grid, f_best_kgpc]}
...
parameter_dict= {'ei_pu':
        ...
        {'random_restarts':random_restarts_carbo, 'num_iter_max':num_iter_max_carbo, 'grid':grid_carbo},
         ...
         {'random_restarts': random_restarts_ei, 'num_iter_max': num_iter_max_ei, 'grid': grid_ei},
                 'kgpc':
         {'num_inner_opt_restarts': num_inner_opt_restarts_kgpc, 'num_outer_opt_restarts': num_outer_opt_restarts_kgpc,
          'num_iter_max': num_iter_max_kgpc, 'grid_opt_in': grid_opt_in_kgpc, 'grid_opt_out': grid_opt_out_kgpc}
                     }
                 # 'pucla_v2':
