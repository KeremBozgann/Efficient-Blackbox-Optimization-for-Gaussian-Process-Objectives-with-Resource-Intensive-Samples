...
def ei_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                 domain, kernel, budget, x0, latent_cost_kernel, num_opt_restarts,  grid, noise=10**(-4),
                    noise_cost= 10**(-4), num_iter_max= 100, plot=False, plot_cost= False, num_layer= None,
                        num_dense= None, num_epoch= 1.7, X_init= None, Y_init= None, Y_cost_init= None,
                            hyper_opt_per= False, plot_color= False):
    ...
    while C<budget:
        ...
        if Xt.shape[0]== 0:
            ...
            xt, xt_val = opt.maximize_ei(kernel, Xt, noise, model, domain, num_opt_restarts, f_best,
...
def test_synthetic():
    ...
    model ,kernel= sin_find_best_suited_kernel(X, Y, noise= noise)

    X_cost, Y_cost= exp_cos_1d_plots(disc, False)
    latent_cost_model , latent_cost_kernel= exp_cos_1d_find_best_suited_kernel(X_cost, Y_cost, noise= noise_cost)
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list, latent_cost_model, f_best_list= \
            ei_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
              ...
              noise_cost= noise_cost, num_iter_max=100, plot=True, plot_cost=True, num_layer=None,
...
def test_synthetic2():
    ...
    model ,kernel= branin_find_best_suited_kernel(X, Y, noise=noise)

    X_cost, Y_cost= exp_cos_2d_plots(disc, False)
    latent_cost_model , latent_cost_kernel= exp_cos_2d_find_best_suited_kernel(X_cost, Y_cost, noise= noise_cost)
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list, latent_cost_model= ei_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
              domain, kernel, budget, x0, latent_cost_kernel, num_opt_restarts, grid, noise= noise,
              noise_cost= noise_cost, num_iter_max=100, plot=True, plot_cost=False, num_layer=None,
              num_dense=None, X_init=None, Y_init=None, Y_cost_init=None, hyper_opt_per=False)
...
def test_quad_2d():
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list, latent_cost_model, f_best_list= \
        ei_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
              ...
              noise_cost= noise_cost, num_iter_max=num_iter_max, plot=False, plot_cost=False, num_layer=None,
...
def test_cifar():
    ...
    budget= 1800; num_opt_restarts= 10;
    grid= False
