...
from util import initial_training_synthetic
...
y_true_opt, x_true_opt, domain =griewank_opt(D)
# X_kern, Y_kern = griewank_plots(disc, plot=False)
# model, kernel = griewank_find_best_suited_kernel(X_kern, Y_kern, noise=noise)
...
for i in range(total_iterations):
    ...
    if objective_func=='fashion':
        ...
        x0 = None
        X, Y, Y_cost= initial_training_synthetic(domain, num_init_train_samples, objective_func, cost_function)
    ...
    latent_cost_model_eiw_eipu, f_best_list_eiw_eipu = \
        eiw_eipu_bo(D, objective_func, cost_function, y_true_opt, x_true_opt,
                    ...
                    num_dense=None,
                    num_epoch= None, X_init=X, Y_init= Y, Y_cost_init=Y_cost,  hyper_opt_per=hyper_opt_per,
                    plot_color=False,
