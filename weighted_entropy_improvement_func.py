...
def WEI(Xt, Yt, X,  kernel, num_samples, noise):
    ...
    ent_reduct_list= np.empty([X.shape[0],1])
    ...
    for i in range(X.shape[0]):
        ...
        temp2= tf.matmul(tf.matmul(kernel.K(X,Xt_psed),tf.linalg.inv(tf.add(kernel.K(Xt_psed,Xt_psed), tf.cast(noise*tf.eye(Xt_psed.shape[0]),tf.float64)),tf.float64)), tf.transpose(kernel.K(X,Xt_psed)))

        ent_prior= 1/2*(2*np.pi*np.e)**(Xt.shape[0])*np.linalg.det(kernel.K(X,X)- temp1); ent_psed= 1/2*(2*np.pi*np.e)**(Xt.shape[0])*np.linalg.det(kernel.K(X,X)- temp2)
        entropy_reduct_x= ent_prior- ent_psed

        ent_reduct_list[i,0]= (entropy_reduct_x)

    weighted_ent_imp= MC_probabilities_of_being_minimum* ent_reduct_list

    return weighted_ent_imp
...
def WEI_bo(X, Y, kernel, budget, index0, noise= 10**(-4), num_mc_samples= 100, plot=False):
    ...
    while t<budget:
        ...
        '''update posterior mean and var functions'''
        u, var = model.predict(X)

        '''plot if 1d input data'''
        if X.shape[1]==1:
            plot_1D(Xt, Yt, xt, yt, X, u ,var)
...
loss_list, Xt, Yt, model= WEI_bo(X, Y, kernel, budget, index0, noise= 10**(-4), num_mc_samples= 100, plot=False)