...
def synthetic_2parts(X):

    x1= X[:,0].reshape(-1,1); x2= X[:,1].reshape(-1,1)

    result= -np.exp(-(x1**2+x2**2))

    return result

def scipy_minimize_synthetic_2parts():
...
def synthetic_2parts_plots(disc, plot= False):
...
def synthetic_2parts_find_best_suited_kernel(X, Y, noise=10**(-4)):
