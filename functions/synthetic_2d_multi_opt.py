...
def synthetic_multi_2d(X):

    x1= X[:,0].reshape(-1,1); x2= X[:,1].reshape(-1,1)

    result=  -2*np.exp(-2*np.sqrt((x1-2)**2+(x2-2)**2)) -1*np.exp(-2*np.sqrt((x1+2)**2+(x2+2)**2))

    return result

def scipy_minimize_synthetic_multi_2d():
...
def synthetic_multi_2d_plots(disc, plot= False):
...
def synthetic_multi_2d_find_best_suited_kernel(X, Y, noise=10**(-4)):
