...
def EI_weighted_TStdR_with_discard(sigma_x, u_x, kernel, Xt, Yt, X, f_best, noise, model, kapa):
    ...
    '''posterior'''
    sum_of_ei_weighted_stds = np.sum(sigma_x_discarded*EI_x_discarded, axis=0)
    ...
    for i in range(X_discarded.shape[0]):
        ...
        sum_of_ei_weighted_pseudo_stds = np.sum(std_psed*EI_x_discarded, axis=0)

        reduct_sum_of_stds = sum_of_ei_weighted_stds - sum_of_ei_weighted_pseudo_stds
...
def EI_weighted_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa=1, noise= 10**(-4), plot=False):
    ...
    while t<budget:
        ...
        if t==0:
            ...
            Acq, index_undiscarded= EI_weighted_TStdR_with_discard(sigma_X, u_X, kernel, Xt, Yt, X, f_best, noise, model, kapa);
...
# loss_list, Xt, Yt, model= EI_weighted_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa, noise= 10**(-4), plot=True)