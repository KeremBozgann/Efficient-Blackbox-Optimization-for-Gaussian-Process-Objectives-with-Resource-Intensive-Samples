...
def EI_bo(X, Y, kernel, budget, index0, noise= 10**(-4), plot=False):
...
kernel= gp.kernels.RBF(lengthscales= [0.5], variance= 1.0); budget= 10; index0= np.random.choice(np.arange(X.shape[0]))
loss_list, Xt, Yt, model= EI_bo(X, Y, kernel, budget, index0, noise= 10**(-4), plot= True)