...
def TStdR_with_discard(sigma_x, kernel, Xt, Yt, X, noise, model, kapa):

    u, var= model.predict_f(X); std= np.sqrt(var)
    ...
    for i in range(X_discarded.shape[0]):
        ...
        sum_of_pseudo_stds = np.sum(np.sqrt(std_psed), axis=0)
...
def TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa=1, noise= 10**(-4), plot=False):
    ...
    while t<budget:
        ...
        if t==0:
            ...
            Acq, index_undiscarded= TStdR_with_discard(sigma_X, kernel, Xt, Yt, X, noise, model, kapa);
...
# loss_list, Xt, Yt, model= TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa, noise= 10**(-4), plot=True)