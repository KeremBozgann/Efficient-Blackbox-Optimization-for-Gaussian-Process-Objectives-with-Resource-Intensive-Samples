...
def EI_factored_TStdR_with_discard(sigma_x, u_x, kernel, Xt, Yt, X, f_best, noise, model, kapa):
    ...
    EI_x_discarded= np.delete(EI_x, discard_list, 0)

    '''posterior'''
    sum_of_stds = np.sum(sigma_x_discarded, axis=0)
    ...
    for i in range(X_discarded.shape[0]):
        ...
        sum_of_pseudo_stds = np.sum(std_psed, axis=0)

        reduct_sum_of_stds = EI_x_discarded[i,0]*(sum_of_stds - sum_of_pseudo_stds)
...
def EI_factored_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa=1, noise= 10**(-4), plot=False):
    ...
    while t<budget:
        ...
        if t==0:
            ...
            Acq, index_undiscarded= EI_factored_TStdR_with_discard(sigma_X, u_X, kernel, Xt, Yt, X, f_best, noise, model, kapa);
...
# import h5py
#
# with h5py.File('./datasets/2d/gp_sample_rbf_l_0.5_v_1.0.h5', 'r') as hf:
...
#
# index0= 196
# loss_list, Xt, Yt, model= EI_factored_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa, noise= 10**(-4), plot=True)