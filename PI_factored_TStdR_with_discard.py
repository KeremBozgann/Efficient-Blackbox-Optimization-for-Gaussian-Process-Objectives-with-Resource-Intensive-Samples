...
def PI_factored_TStdR_with_discard(sigma_x, u_x, kernel, Xt, Yt, X, f_best, noise, model, kapa):
    ...
    for i in range(X_discarded.shape[0]):
        ...
        reduct_sum_of_stds = PI_x_discarded[i,0]*(sum_of_stds - sum_of_pseudo_stds)
...
def PI_factored_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa=1, noise= 10**(-4), plot=False):
    ...
    while t<budget:
        ...
        if t==0:
            ...
            Acq, index_undiscarded= PI_factored_TStdR_with_discard(sigma_X, u_X, kernel, Xt, Yt, X, f_best, noise, model, kapa);
...
# index0= 196
#
# loss_list, Xt, Yt, model= PI_factored_TStdR_with_discard_bo(X, Y, kernel, budget, index0, kapa, noise= 10**(-4), plot=True)