...
def TStdR(sigma_x, kernel, Xt, Yt, X, noise):


    '''posterior'''
    sum_of_stds = np.sum(sigma_x, axis=0)

    '''posterior with pseudo evaluations'''
    list_reduction_sum_of_variances = np.empty([X.shape[0],1])
    ...
    return list_reduction_sum_of_variances
...
def TStdR_bo(X, Y, kernel, budget, index0, noise= 10**(-4), plot=False):
    ...
    while t<budget:
        ...
        if t==0:
            ...
            Acq= TStdR(sigma_X, kernel, Xt, Yt, X, noise); index_max= np.argmax(Acq,axis=0)
...
# loss_list, Xt, Yt, model= TStdR_bo(X, Y, kernel, budget, index0, noise= 10**(-4))