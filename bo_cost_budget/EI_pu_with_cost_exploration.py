...
def EI_pu_with_cost_exploration_bo(X, Y, Y_cost, kernel, cost_kernel, kapa_cost,
    ...
    if Xt.shape[0]>0:
        ...
        latent_cost_model= gp.models.GRP((Xt, log_Yt_cost), cost_kernel)
        latent_cost_model.likelihood.variance.assign(noise_cost)
    ...
    while C<budget:
        ...
        if Xt.shape[0]==0:
            ...
            Acq =EI_pu_with_cost_exploration(u_X, sigma_X, u_latent_cost, sigma_latent_cost,
                                               f_best, cost_kernel, noise_cost, Xt, Yt_cost, X, kapa_cost)

            Acq_dict= {'EI_pu_with_cost_exploration':Acq, 'carbo':Acq_carbo, 'EI_pu': Acq_EI_pu, 'EI': Acq_EI, 'utlc':Acq_utlc}
...
def test_ei_pu_with_cost_exploration(dimension, ls, var, ls_cost, var_cost,
                                     rang, disc, kernel_type, cost_kernel_type, budget, index0, kapa_cost):
    # dimension= 1
    ...
    noise_cost= 10**(-4); noise= 10**(-4)

    loss_list, Xt, Yt, model, cost_list, cum_cost_list= EI_pu_with_cost_exploration_bo(X, Y, Y_cost, kernel,
                                       cost_kernel, kapa_cost,  budget, index0, noise= noise,
                                                   noise_cost= noise_cost, plot=True, plot_cost=True)
...
# rang= 10;  disc= 100; budget= 5; index0= np.random.choice(np.arange(disc)); kapa_cost= 1.0
#
# loss_list, Xt, Yt, model, cost_list, cum_cost_list=  \
#     test_ei_pu_with_cost_exploration(dimension, ls, var, ls_cost, var_cost,
#                                       rang, disc, kern_type, kern_type_cost, budget, index0, kapa_cost)