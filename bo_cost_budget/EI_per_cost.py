...
def EI_pu_with_cost_exploration(u_X, sigma_X, u_latent_cost, sigma_latent_cost,
                                               f_best, kernel_cost, noise_cost, Xt, Yt_cost, X, kapa_cost):
...
def EI_per_cost_bo(X, Y, Y_cost, kernel, cost_kernel, budget, index0, noise=10**(-4), noise_cost= 10**(-4), plot=False, plot_cost= False):
    ...
    while C<budget:
        ...
        if t==0:
            ...
            Acq= EI_per_cost(sigma_X, u_X , f_best, u_cost); index_max= np.argmax(Acq,axis=0)
        ...
        if t>0:
            ...
            if plot_cost == True:
                ...
                # if X.shape[1]==1:
                #     cost_posterior_chosen_point_evaluated_points(X, Y_cost, Xt, Yt_cost, xt_cost, yt_cost, u_cost, sigma_cost )
                #     input('press a key to continue')
...
#
def test_ei_pu(dimension, ls, ls_cost, var, var_cost,noise,
               noise_cost, budget, index0, rang, disc, kern_type, cost_kern_type):
    ...
    with h5py.File('../datasets/cost_{}d/gp_cost_sample_{}_l_{}_v_{}_rang_{}_disc{}.h5'. \
                           format(dimension, cost_kern_type, ls_cost, var_cost, rang, disc), 'r') as hf:
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list= EI_per_cost_bo(X, Y, Y_cost, kernel, cost_kernel, budget, index0, noise= noise, noise_cost= noise_cost, plot= True, plot_cost= True)

    return loss_list, Xt, Yt, model, cost_list, cum_cost_list


# dimension=1; budget=10; rang=10; disc= 100; ls= 0.5; var= 1.0; ls_cost= 1.0; var_cost= 1.0; noise= 10**(-4);
# noise_cost= 10**(-4)
# kern_type= 'rbf'; cost_kern_type= 'rbf';
# index0= np.random.choice(np.arange(disc**dimension))
#
# loss_list, Xt, Yt, model, cost_list, cum_cost_list= test_ei_pu(dimension, ls, ls_cost, var, var_cost,noise,
#                noise_cost, budget, index0, rang, disc, kern_type, cost_kern_type)
