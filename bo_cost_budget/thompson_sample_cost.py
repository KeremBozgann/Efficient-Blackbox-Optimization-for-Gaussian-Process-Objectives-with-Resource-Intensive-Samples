...
def thompson_sample_cost_bo(X, Y, Y_cost, kernel, cost_kernel, kapa_cost,
    ...
    while C<budget:
        ...
        if Xt.shape[0]==0:
            ...
            Acq_dict= {'thompson_sample_cost':Acq, 'carbo':Acq_carbo, 'EI_pu': Acq_EI_pu, 'EI': Acq_EI, 'utlc':Acq_utlc,
                            'Acq_pu_with_cost_exploration':Acq_pu_with_cost_exploration}
...
def test_thompson_sample_cost_bo(dimension, ls, var, ls_cost, var_cost,
    ...
    loss_list, Xt, Yt, model, cost_list, cum_cost_list= thompson_sample_cost_bo(X, Y, Y_cost, kernel, cost_kernel,
                           kapa_cost,budget, index0, num_samp, noise=10**(-4),
                                noise_cost= 10**(-4), plot=True, plot_cost=True)

    return loss_list, Xt, Yt, model, cost_list, cum_cost_list

#
# dimension= 1; kernel_type= 'rbf'; cost_kernel_type= 'rbf'; ls= 0.5; ls_cost= 1.0; var= 1.0; var_cost= 1.0;
# rang= 10;  disc= 100; budget= 5; num_samp= 200; index0= np.random.choice(np.arange(disc)); kapa_cost= 1.0
#
# loss_list, Xt, Yt, model, cost_list, cum_cost_list= \
#     test_thompson_sample_cost_bo(dimension, ls, var, ls_cost, var_cost,
#              rang, disc, kernel_type, cost_kernel_type, budget, index0, kapa_cost, num_samp)